...
(define-teach intermediate foldr
  (lambda (f e . l)
    ...
    (unless (and (procedure? f) (procedure-arity-includes? f (+ LE 1)))
      (hocheck 'foldr "first argument must be a function that expects ~a arguments, given ~e" LE f))
...
(define-teach intermediate foldl
  (lambda (f e . l)
    ...
    (unless (and (procedure? f) (procedure-arity-includes? f (+ LE 1)))
      (hocheck 'foldl "first argument must be a function that expects ~a arguments, given ~e" LE f))
    (for ([l l] [i (in-naturals)])
         (unless (beginner-list? l) 
